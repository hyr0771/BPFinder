...
| mergingStrategy | Three merging rules for branched compound:**overlapping,non-overlapping,default**. The default merging strategy means that BPFinder will first search the branched pathways by non-overlapping rule, and then BPFinder will search the branched pathways by overlapping rule in the case of no branched pathways are returned by non-overlapping rule.| default |
